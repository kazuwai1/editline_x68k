...
/*     DIRENTRY    *ep; */
    ...
    size_t      choices;
    size_t      total;
    struct FILBUF dosbuf;
    static unsigned char dosfname[256+1];
    ...
    if ( strlen(dir) > 256-23 )
        return 0;	/* too long directory name */

    if ( dir[0] == '.' && dir[1] == '\0' ) {
        strcpy( dosfname, "*.*" );
    } else {
/*        sprintf(dosfname, "%s\\%s*.*", dir, file); */
        strcpy( dosfname, dir );
        strcat( dosfname, "\\" );
        strcat( dosfname, file );
        strcat( dosfname, "*.*");
    }

    if ( 0 < FILES( &dosbuf, dosfname, 0x37 ) )
        return 0;
...
#elif defined(X68LIBC)	/* for X68000 (C89/LIBC) */
                sprintf(path, "%s/%s", dir, av[0]);
#else /* for X68000 (C89/XC)) */
                sprintf(path, "%s\\%s", dir, av[0]);
#endif
                rl_add_slash(path, p);
                free(path);
