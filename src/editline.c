...
#include <sys/ioctl.h>
#else /* for X68000 */
/* #include <sys/ioctl.h> */ /* no-need for X68000 */
#include <doslib.h>
#endif
...
int rl_getc(void)
{
    ...
    do {
        c0 = K_KEYSNS();	/* prefetch */
        g6 = K_KEYBIT(6);	/* status of keygroup 6 */
        g7 = K_KEYBIT(7);	/* status of keygroup 7 */
    } while ( c0 == 0 );
    ...
    if ( (g7 & 0x80) ) c = 0x0c;	/* CLR is ^L */
    else if ( (g7 & 0x40) ) c = 0x0e;	/* down arrow is ^N */
    else if ( (g7 & 0x20) ) c = 0x06;	/* right arrow is ^F */
    else if ( (g7 & 0x10) ) c = 0x10;	/* up arrow is ^P */
    else if ( (g7 & 0x08) ) c = 0x02;	/* left arrow is ^B */
    else if ( (g7 & 0x02) ) c = 0x10;	/* roll down is ^P */
    else if ( (g7 & 0x01) ) c = 0x0e;	/* roll up is ^N */
...
#if !defined(human68k) || defined(X68LIBC)	/* no-need for X68000 (XCLIB) */
        if (*p == '\\') {
